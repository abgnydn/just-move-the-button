...
An interactive comic (guess → reveal, one page at a time) that shows non-technical people the invisible work behind a "simple" software change. Comic-book style: bold ink, halftone, captions, SFX, splash panels. The reader plays as the developer (optionally named) or the ticket writer through a bad Tuesday.
